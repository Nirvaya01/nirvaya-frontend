...
import { sendSosAlert } from '../constants/api';
...
export default function SosConfirmation() {
  ...
  async function handleSend() {
    if (sending || sent || cancelledRef.current) return;
    if (!coords) {
      ...
      <View style={styles.card}>
        ...
        {!sent && (
          <View style={styles.countdownWrap}>
            <Text style={styles.countdownNumber}>{secondsLeft > 0 ? secondsLeft : 0}</Text>
          </View>
        )}

        <CurrentLocationMap status={status} coords={coords} errorMessage={errorMessage} />
...
const styles = StyleSheet.create({
  ...
  card: { width: '100%', backgroundColor: '#fff', borderRadius: 24, padding: 24, overflow: 'hidden' },
  topBar: { position: 'absolute', top: 0, left: 0, right: 0, height: 6, backgroundColor: '#e15347' },
  title: { fontSize: 26, fontWeight: '800', color: '#0d1c2f', textAlign: 'center', marginTop: 12 },
  subtitle: { fontSize: 15, color: '#45474c', textAlign: 'center', marginTop: 8 },
  countdownWrap: {
    alignSelf: 'center',
    width: 140,
    height: 140,
  ...
  countdownNumber: { fontSize: 40, fontWeight: '800', color: '#e15347' },
  locationCaption: { fontSize: 13, color: '#45474c', marginTop: 12, marginBottom: 8 },
  errorText: { color: '#ba1a1a', fontSize: 13, fontWeight: '600', marginTop: 8, textAlign: 'center' },
  cancelBtn: {
    borderWidth: 1.5,
    borderColor: '#0d1c2f',
    borderRadius: 999,
  ...
  cancelBtnText: { fontWeight: '700', color: '#0d1c2f', fontSize: 15 },
  sendBtn: {
    flexDirection: 'row',
    gap: 8,
